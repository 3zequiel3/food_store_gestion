...
Define the `ingredients` capability for the Food Store backend. Ingredients are atomic food items that can be marked as allergens, exposed via a REST API, and associated with products through the `product_ingredients` pivot table. This capability builds on the `catalog` capability (which defines the `Ingrediente` ORM model and the `ingredients` DB table) and does not require additional persistence — only the application service layer, API endpoints, and tests.
