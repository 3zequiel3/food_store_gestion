...
- **THEN** `useCartStore` saves the items array to `localStorage` key `food-store-cart` and the cart survives page refresh, browser close, and `useAuthStore.logout()` (RN-CR02)
