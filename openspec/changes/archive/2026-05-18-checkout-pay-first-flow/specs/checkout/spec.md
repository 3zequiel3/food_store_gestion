...
#### Scenario: Cambio posterior de precio no altera snapshot
- **WHEN** un cliente crea un pedido vía checkout con un producto a `100.00`
- **AND** un admin actualiza el precio a `150.00` después
- **THEN** `DetallePedido.precio_snapshot` del pedido original sigue siendo `100.00`

#### Scenario: Snapshot de dirección preserva datos al momento del pedido
- **WHEN** un cliente crea un pedido con `direccion_id=42`
- **AND** el cliente edita esa dirección después
- **THEN** `orders.direccion_snapshot` conserva el texto original
