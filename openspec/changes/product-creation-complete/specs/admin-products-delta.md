# Delta Spec: product-creation-complete → admin-products

## MODIFIED Requirements

### Requirement: Admin puede crear un producto (MODIFIED)
**CHANGE**: The product creation modal now includes category selection, ingredient assignment, and image management in a 2-column layout: left column for data fields + selectors, right column for image management. `categoria_ids` is REQUIRED — the form SHALL NOT allow submission without at least one leaf category selected. The same modal is used for both create and edit operations (unified form).

(Previously: modal had only basic fields — nombre, precio, stock, descripción, disponible)
...
**CHANGE**: The edit modal now preloads categories, ingredients, and images. The admin can modify all aspects of the product in the same unified form.

(Previously: modal only edited basic fields)
