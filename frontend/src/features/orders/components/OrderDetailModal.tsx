...
import { X, Package, Ban, AlertCircle, MapPin, CreditCard, Banknote, FileText } from 'lucide-react';
...
export function OrderDetailModal({ orderId, isAdmin = false, onClose }: OrderDetailModalProps) {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-2xl max-h-[92vh] sm:max-h-[90vh] flex flex-col rounded-t-2xl sm:rounded-2xl bg-background/95 backdrop-blur-2xl border border-glass-border shadow-2xl shadow-black/20 overflow-hidden"
      ...
      >
        ...
        {/* Header */}
        <div className="shrink-0 flex items-center gap-3 px-5 py-4 border-b border-glass-border bg-background/60 backdrop-blur-xl">
          ...
          <div className="flex-1 min-w-0">
            ...
            {order && (
              <p className="text-xs text-muted-foreground mt-0.5">{formatDate(order.creado_en)}</p>
            )}
          </div>
          ...
          <button
            type="button"
            onClick={onClose}
            className="shrink-0 rounded-lg p-1.5 text-muted-foreground hover:bg-glass-hover hover:text-foreground transition-colors"
            aria-label="Cerrar"
          >
            <X className="h-4 w-4" />
          </button>
          ...
          {order && (
            <>
              ...
              {isBlockedFromCancel && (
                <div className="flex items-start gap-2.5 rounded-xl bg-muted/20 border border-glass-border px-4 py-3">
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Tu pedido ya está en preparación y no puede ser cancelado. Contactanos si
                    necesitás ayuda.
                  </p>
                </div>
              )}
              ...
              <div className="rounded-xl bg-glass/40 border border-glass-border overflow-hidden">
                ...
                <div className="grid grid-cols-2 gap-0 divide-x divide-y divide-glass-border">
                  <InfoCell
                    icon={
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="rounded-xl bg-glass/40 border border-glass-border p-4">
                  <OrderTimeline
...
function ModalSkeleton() {
  return (
    <div className="flex flex-col gap-4 py-2">
      <div className="h-16 rounded-xl bg-gradient-to-r from-muted/50 via-muted to-muted/50 animate-shimmer bg-[length:200%_100%]" />
      <div className="grid grid-cols-2 gap-3">
        {[...Array(4)].map((_, i) => (
          <div
            key={i}
            className="h-14 rounded-xl bg-gradient-to-r from-muted/50 via-muted to-muted/50 animate-shimmer bg-[length:200%_100%]"
          />
        ))}
      </div>
